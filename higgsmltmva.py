...
if dotraining:
    ...
    #method = factory.BookMethod(TMVA.Types.kBDT, "BDT") #default parameter gives 3.24954 sigma
    method = factory.BookMethod(TMVA.Types.kBDT, "BDT",
                       ":".join([#specify BDT parameters
                       "!H",
                       "!V",
                       "NTrees=850",#actually it doesn't matter if around 1000
                       #"nEventsMin=150",
                       'MinNodeSize=0.600005%',#ROOT 5.24 replaced nEventsMin with MinNodeSize, 
                       #'MinNodeSize=0.20005%',
                       "MaxDepth=4",#the deeper, the higher significance in validation, but dangerous overfit
                       "BoostType=AdaBoost",
                       #"BoostType=Bagging",#the default parameter for bagging doesn't work :-(
                       #"AdaBoostBeta=0.5",
                       "AdaBoostBeta=0.85",#higher learning rate actually works better
